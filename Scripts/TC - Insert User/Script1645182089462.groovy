...
response = WS.sendRequest(findTestObject('User/Insert User', [('Name') : Name, ('Email') : Email, ('Gender') : Gender, ('Status') : Status]))

if (WS.verifyResponseStatusCode(response, 201, FailureHandling.OPTIONAL)) {
    response2 = WS.sendRequest(findTestObject('User/Get User', [('Email') : Email]))
...
//Bedain if buat Positive TC dan Negative TC

if(WS.verifyResponseStatusCode(response, 422, FailureHandling.OPTIONAL)){
	print("Email Cannot Be Duplicate!")
}
